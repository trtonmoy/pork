...
(function ($) {
  ...
  function commarize(data) {
    ...
    return Math.abs(Number(data).toFixed(2)) >= 1.0e15
      ? (Math.abs(Number(data)) / 1.0e12).toFixed(2) + formats[0]
      : Math.abs(Number(data)) >= 1.0e9
      ? (Math.abs(Number(data)) / 1.0e9).toFixed(2) + formats[1]
      : Math.abs(Number(data)) >= 1.0e6
      ? (Math.abs(Number(data)) / 1.0e6).toFixed(2) + formats[2]
      : Math.abs(Number(data)) >= 1.0e3
      ? (Math.abs(Number(data)) / 1.0e3).toFixed(2) + formats[3]
  ...
  if ($(".mcwp-crypto").length > 0) {
    $.fn.extend({
      animateCss: function (animationName, callback) {
        ...
        this.addClass("mcwp-animated " + animationName).one(
          animationEnd,
          function () {
      ...
      var opts = $.extend(
        {
          ...
          bgColor:
            this.data("bgcolor") == "" ? "#dd3333" : this.data("bgcolor"),
        },
        options
      );
        ...
        columnDefs: [
          ...
          },
          {
            ...
            render: function (data, type, row, meta) {
              var tabletd;
              tabletd =
                '<div class="mcwp-card-head"><div><img src="' +
                row.imgpath +
                '" class="mcwp-coinimage"><p>';
              if (typeof row.link != "undefined" && row.link === true) {
                tabletd =
                  tabletd +
                  '<a rel="nofollow" href="https://coingecko.com/coins/' +
                  row.cid +
                  '" target="_blank">' +
                  data +
                  "</a>";
          ...
          },
          {
            ...
            render: function (data, type, row, meta) {
              ...
              if (num >= 1 || num == 0) {
                ...
                zerocount =
                  num.toString().length -
                  Number(num.toString().split(".")[1]).toString().length -
                  2;
  ...
  $(window).load(function () {
    $(".cc-stats").each(function () {
      ...
      if (mult > 0.5) {
        $(this)
          .find(".cc-dup")
          .append(clonedElem.multiply(Math.ceil(mult)));
      } else {
